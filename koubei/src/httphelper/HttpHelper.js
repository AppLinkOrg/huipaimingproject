...
export class HttpHelper {
  ...
  static Post(url, data) {
    ...
    return axios
      .post(fullurl, HttpHelper.ParamUrlencoded(data), {
        headers: {
  ...
  static async AsyncPost(url, data) {
    try {
      var fullurl = Config.ApiUrl + url
      const token = window.localStorage.getItem('token')
      const tokenkey = window.sessionStorage.getItem('v')
      console.log('fullurl', fullurl)

      const response = await axios.post(fullurl, HttpHelper.ParamUrlencoded(data), {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'Access-Control-Allow-Origin': '*',
          Sign: HttpHelper.Sign,
  ...
  static getNowDate() {
    ...
    var currentdate =
      year + sign1 + month + sign1 + day + ' ' + hour + sign2 + minutes + sign2 + seconds
    return currentdate
  }
